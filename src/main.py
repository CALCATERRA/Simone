...
# === Funzione per leggere il prompt personalizzato ===
def get_prompt():
    try:
        with open("prompt.txt", "r", encoding="utf-8") as file:
            return file.read().strip()
    except Exception as e:
        print(f"[ERRORE] Lettura prompt.txt fallita: {e}")
...
def get_messenger_messages():
    print("[INFO] Recupero messaggi Messenger...")
    ...
    if response.status_code == 200:
        ...
        print(f"[ERRORE] Messenger API: {response.text}")
        return None


# === Funzione per ottenere i messaggi da Instagram ===
def get_instagram_messages():
    print("[INFO] Recupero messaggi Instagram...")
    ...
    if response.status_code == 200:
        ...
        print(f"[ERRORE] Instagram API: {response.text}")
        return None


# === Funzione per inviare un messaggio a OpenAI ===
...
def send_messenger_reply(user_id, text):
    ...
    if response.status_code != 200:
        print(f"[ERRORE] Invio Messenger: {response.text}")


# === Invio risposta su Instagram ===
def send_instagram_reply(user_id, text):
    ...
    response = requests.post(url, json=data)
    if response.status_code != 200:
        print(f"[ERRORE] Invio Instagram: {response.text}")


# === Funzione principale per Appwrite ===
def main(context):
    # === Webhook di verifica Meta ===
        ...
        for conv in messenger_data["data"]:
            if "messages" in conv:
                for msg in conv["messages"]["data"]:
                    ...
                    if user_message:
                        print(f"[Messenger] Msg da {user_id}: {user_message}")
                        reply = send_message_to_openai(user_message)
                        send_messenger_reply(user_id, reply)

    # === Gestione messaggi Instagram ===
        ...
        for conv in instagram_data["data"]:
            if "messages" in conv:
                for msg in conv["messages"]["data"]:
                    ...
                    user_message = msg.get("message")
                    if user_message:
                        print(f"[Instagram] Msg da {user_id}: {user_message}")
                        reply = send_message_to_openai(user_message)
